...
public class JxcSynTradeorderRefundProcessLister implements TradeorderRefundProcessLister, TradeOrderRefundsListener {
		...
		try {
			...
			if (tradeOrderContext.getTradeOrderRefunds().getRefundsStatus() == RefundsStatusEnum.WAIT_SELLER_VERIFY) {
				sendMQMessage(TradeOrderMQMessage.TOPIC_ORDER_SYNC, buildOnlineOrder(tradeOrderContext));
					...
					|| tradeOrderContext.getTradeOrderRefunds()
							.getRefundsStatus() == RefundsStatusEnum.WAIT_BUYER_RETURN_GOODS
					|| tradeOrderContext.getTradeOrderRefunds()
							.getRefundsStatus() == RefundsStatusEnum.WAIT_SELLER_TAKE_GOODS
					|| tradeOrderContext.getTradeOrderRefunds()
							.getRefundsStatus() == RefundsStatusEnum.WAIT_SELLER_REFUND
					|| tradeOrderContext.getTradeOrderRefunds()
							.getRefundsStatus() == RefundsStatusEnum.SELLER_REJECT_REFUND
					|| tradeOrderContext.getTradeOrderRefunds()
							.getRefundsStatus() == RefundsStatusEnum.SELLER_REJECT_APPLY
					|| tradeOrderContext.getTradeOrderRefunds()
							.getRefundsStatus() == RefundsStatusEnum.YSC_REFUND_SUCCESS
					|| tradeOrderContext.getTradeOrderRefunds().getRefundsStatus() == RefundsStatusEnum.YSC_REFUND
					|| tradeOrderContext.getTradeOrderRefunds()
							.getRefundsStatus() == RefundsStatusEnum.BUYER_REPEAL_REFUND
					|| tradeOrderContext.getTradeOrderRefunds()
							.getRefundsStatus() == RefundsStatusEnum.CUSTOMER_SERVICE_CANCEL_INTERVENE
					|| tradeOrderContext.getTradeOrderRefunds()
							.getRefundsStatus() == RefundsStatusEnum.FORCE_SELLER_REFUND_SUCCESS) {
		...
		if (CollectionUtils.isNotEmpty(tradeOrderRefundsItems)) {
			...
			for (TradeOrderRefundsItem item : tradeOrderRefundsItems) {
				...
				if (item.getQuantity() != null && item.getQuantity().intValue() > 0) {
					actualPrice = item.getUnitPrice().subtract(storePreferentialPrice
							.divide(new BigDecimal(item.getQuantity()), 4, BigDecimal.ROUND_HALF_UP));
				} else if (item.getWeight() != null && storePreferentialPrice.compareTo(BigDecimal.ZERO) == 1) {
					actualPrice = item.getUnitPrice()
							.subtract(storePreferentialPrice.divide(item.getWeight(), 4, BigDecimal.ROUND_HALF_UP));
				}
	...
	private OnlineOrderVo buildOnlineOrderVo(TradeOrderContext tradeOrderContext) {
		...
		vo.setOrderNo(tradeOrderRefunds.getRefundNo());
		vo.setLogisticsType(
				tradeOrderRefunds.getLogisticsType() == null ? 2 : tradeOrderRefunds.getLogisticsType().ordinal());// 为空传2
		...
		/*
		 * 0:等待卖家确认,1:买家撤销退款,2:等待买家退货,3:等待卖家收货,4:待卖家退款,5:卖家拒绝退款, 6:退款成功,7:卖家拒绝申请,8:申请客服介入,9:客户介入取消,10:友门鹿退款,
		...
		/*
		 * 我们的值 左文明的值 6:退款成功 0-成功 0:等待卖家确认 1-待发货/等待卖家确认 2:等待买家退货 2-发货/等待买家退货 3:等待卖家收货,4:待卖家退款 3-确认收货/等待卖家退款 5:卖家拒绝退款,
		 * 7:卖家拒绝申请 4-拒收/卖家拒绝退货 12:待卖家退款(强制),13:卖家退款成功(强制) 5-强制卖家退款 10:友门鹿退款,11:(纠纷单)友门鹿退款成功 6-强制友门鹿退款 1:买家撤销退款
		 * 9:客户介入取消, 8-取消
		 */
		...
		while (itemIt.hasNext()) {
			...
			if (item.getSpuType() == SpuTypeEnum.assembleSpu) {
				...
				for (TradeOrderComboSnapshot comboDetail : comboDetailList) {
					...
					favourItem = comboDetail.getOnlinePrice().subtract(comboDetail.getUnitPrice())
							.multiply(BigDecimal.valueOf(splitItem.getQuantity()));
	...
	private List<TradeOrderComboSnapshot> findComboDetailList(List<TradeOrderComboSnapshot> comboSkuList,
			String comboSkuId) {
