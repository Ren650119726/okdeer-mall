...
/**
 * ClassName: ActivityAppRecommendGoodsServiceTest 
...
public class ActivityAppRecommendGoodsServiceTest {

	/** 日志对象 */
	private static final Logger log = LoggerFactory.getLogger(ActivityAppRecommendGoodsServiceTest.class);

	@Autowired
	private ActivityAppRecommendGoodsService recommendGoodsService;

	@Test
	public void test1InsertMore() {
		try {
			log.info("测试批量APP端服务商品推荐与商品关联数据");
			List<ActivityAppRecommendGoods> list = new ArrayList<ActivityAppRecommendGoods>();
			ActivityAppRecommendGoods recommend = null;
			for (int i = 1; i < 10; i++) {
				recommend = new ActivityAppRecommendGoods();
	...
	public void test2FindList() {
		try {
			...
			List<ActivityAppRecommendGoods> list = recommendGoodsService.findList(paramDto);
	...
	public void test3FindListByRecommendId() {
		try {
			log.info("测试获取APP端服务商品推荐与商品关联列表");
			List<ActivityAppRecommendGoods> list = recommendGoodsService.findListByRecommendId("1");
			Assert.assertNotNull(list);
		} catch (Exception e) {
			log.error("查询获取APP端服务商品推荐与商品关联列表异常:{}", e);
		}
